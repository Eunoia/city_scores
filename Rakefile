...
task :enqueue_cost_lookups => :environment do
	...
	join stations on ST_DWithin(subj.geog, stations.geog, 6400, true) 
	where subj.id != stations.id and subj.geog <-> stations.geog > 3000 order by subj.geog<->stations.geog desc
	SQL
